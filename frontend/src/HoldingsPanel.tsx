...
export default function HoldingsPanel({ token }: HoldingsPanelProps) {
  ...
  return <section className="panel holdings-panel">
    ...
    {holdings.length > 0 && <div className="holdings-list">{holdings.slice(0, 4).map((item) => <div className="holding-row" key={item.id}><span><strong>{item.name}</strong><small>{item.symbol}{item.target_return != null ? ` · 目标 ${(item.target_return * 100).toFixed(1)}%` : ""}{item.max_loss != null ? ` · 最大亏损 ${(item.max_loss * 100).toFixed(1)}%` : ""}</small></span><span>{item.quantity.toLocaleString("zh-CN")} 股</span><strong>¥{(item.quantity * item.cost_price).toLocaleString("zh-CN", { maximumFractionDigits: 2 })}</strong><button className="holding-remove" onClick={() => remove(item)}>删除</button></div>)}</div>}
  </section>;
}
