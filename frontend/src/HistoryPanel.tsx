...
export default function HistoryPanel({ token, symbol }: HistoryPanelProps) {
  ...
  return <section className="history-panel panel"><div className="panel-header compact"><div><div className="section-kicker"><AreaChart size={14}/>历史走势</div><h2>{symbol} <span className="ticker">{status}</span></h2><p>收盘价与成交量 · 数据来自当前配置 Provider</p></div></div>{!token && <p className="history-empty">登录后查看真实历史行情，演示区不绘制虚假曲线。</p>}{token && !path && <p className="history-empty">{status}</p>}{path && <><div className="history-meta"><span>最新 <strong>¥{latest?.toFixed(2)}</strong></span><span className={change != null && change >= 0 ? "history-positive" : "history-negative"}>{change != null ? `${change >= 0 ? "+" : ""}${(change * 100).toFixed(2)}%` : "—"} 区间</span><span>资金流 {hasFlow ? "可用" : "未提供"}</span></div><svg className="history-chart" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={`${symbol} 历史收盘价与成交量曲线`}>{points.map((point, index) => <rect key={`${point.time}-volume`} className="volume-bar" x={(index / points.length * 100).toFixed(2)} y={(100 - point.volume / maxVolume * 28).toFixed(2)} width={(96 / points.length).toFixed(2)} height={(point.volume / maxVolume * 28).toFixed(2)}/>) }<path d={path} fill="none" stroke="#c95750" strokeWidth="2" vectorEffect="non-scaling-stroke"/></svg><div className="history-foot"><span>{points.length} 个数据点 · {hasFlow ? "资金流已返回" : "资金流未提供"}</span><span>{new Date(points[0].time).toLocaleDateString("zh-CN")} – {new Date(points.at(-1)!.time).toLocaleDateString("zh-CN")}</span></div></>}</section>;
}
